...
try:
    # qmod/paths.py
    from .paths import artifacts_dir         # stable, absolute-ish output dir
...
def optimize_with_optuna(
    ...
    *,
    ...
    """
    Run an Optuna study and return best_params (dict).

    Designed to be called from run_full_composite:

        _optuna_imp.optimize_with_optuna(objective, n_trials=n_trials, vis_dir=...)

    - Does *not* assume a particular config system, but will read
      `optuna_settings` from cfg_mod.CONFIG if available.
    - Paths are created under artifacts_dir("vis_macd", "optuna") by default.
    - Single-process (n_jobs=1) for API safety.
    """
    ...
    # Output folder
    out_dir = Path(vis_dir) if vis_dir else artifacts_dir("vis_macd", "optuna")
    ...
    # IMPORTANT: keep API path stable
    study.optimize(
        objective_func,
